...
int main(int argc, char *argv[])
{
    ...
    while (runTime.run())
    {
      ...
      if (runTime.timeIndex() <= 3)
      {
        ...
        Uold = U; pold = p; Uc = U; dt=runTime.deltaT();
        phi = (fvc::interpolate(U) & mesh.Sf());
        dU = dt*(fvc::laplacian(turbulence->nuEff(), U) - fvc::div(phi,U)); // k_1
      ...
      // ---------------------------------------------------------------------
            ...
            dU_2 = dt*(fvc::laplacian(turbulence->nuEff(), U) - fvc::div(phi,U)); // k_2
            dU_1 = dU;      // guarde en k_1           
            // dU2 = dt*(fvc::laplacian(turbulence->nuEff(), U) - fvc::div(phi,U)); // k_2
            // dU1 = dU;
      ...
      else
      {
        ...
        phi = (fvc::interpolate(U) & mesh.Sf());
        dU1 = dt*(fvc::laplacian(turbulence->nuEff(), U) - fvc::div(phi,U)); // k1
        Uc = Uc + 0.5*dU1; // yn + 0.5*k1
        U = Uold + (5/12)*dU1; // U = Uold + (5/12)*dU1; 
        #include "pressureCorrection.H"

        phi = (fvc::interpolate(U) & mesh.Sf());
        dU2 = dt*(fvc::laplacian(turbulence->nuEff(), U) - fvc::div(phi,U)); // k2
        Uc = Uc + dU2; // (yn + 0.5*k1) + k2 

        Uc = Uc + 0.5*dU_1 - dU_2; // ((yn + 0.5*k1) + k2) + 0.5*k_1 - k_2
        // Uc = Uc + 0.5*dU1.oldTime() - dU2.oldTime();
        U = Uc;
        #include "pressureCorrection.H"
        U = U + deltaP * dt;
        U.correctBoundaryConditions();
